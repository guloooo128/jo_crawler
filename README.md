...
- 🤖 **LLM 智能生成解析器** — 基于可访问性树快照 + refs，LLM 为每个站点生成专用 JS 解析器，内置 AST 验证 + 自动修复 + 生成重试机制
...
- 🔄 **动态加载** — 生成的解析器自动注册，无需重启即可使用；找不到匹配解析器时自动回退到 GenericParser（LLM 在线提取）
- 🧩 **解析器缓存** — 已有解析器自动跳过生成，支持 `--force` 强制重新生成
- 🍪 **弹窗自动处理** — 多策略自动关闭 Cookie 横幅、隐私弹窗（CSS 选择器 + 文本匹配）
- 🕵️ **反爬对策** — 内置浏览器指纹伪装（禁用 AutomationControlled、自定义 UA、伪造 plugins/languages）
- 🔌 **CDP 连接** — 支持 `--cdp` 连接已打开的 Chrome 实例，绕过复杂反爬检测
- 📜 **懒加载支持** — 生成阶段自动滚动页面触发懒加载内容，确保快照完整；HTML 回退检测职位链接
- 💾 **SQLite 持久化** — WAL 模式高性能写入，自动去重，支持按来源/公司/关键字查询导出
- 🔁 **LLM 重试** — 指数退避自动重试，AST 验证失败时将错误反馈给 LLM 重新生成
...
# 数据库配置
DATABASE_PATH=output/jobs.db    # 可选，默认 output/jobs.db
...
npm run crawl -- --cdp 9222        # 连接已打开的 Chrome（绕过反爬）
```
...
选项：
  ...
  --csv                   使用 CSV 配置文件
  --txt                   使用 TXT 配置文件
  --cdp <url>             连接已有 Chrome（CDP 端口或 ws:// URL），绕过反爬
...
选项：
  ...
  -c, --concurrency <number> 并发数（1=串行优化模式，>1=并行浏览器池）[默认: 1]
...
选项：
  -o, --output <path>      输出文件路径（默认根据格式自动生成）
  -f, --format <format>    输出格式 (json|csv) [默认: json]
  -s, --source <source>    按来源过滤（模糊匹配）
  -c, --company <company>  按公司过滤（模糊匹配）
  -k, --keyword <keyword>  按关键字过滤（匹配标题和描述）
  -l, --limit <number>     最大导出条数
  --db <path>              数据库文件路径 [默认: output/jobs.db]
  --fields <fields>        导出字段（逗号分隔），默认全部
  --no-desc                不导出 description 字段（减小文件体积）
```

> **注意**: `generate` 命令会自动检测 `links.csv`（优先）或 `links.txt`；`crawl` 命令默认读取 `links.txt`，需加 `--csv` 才使用 CSV 配置。

> **CDP 连接**: 支持三种格式 — 端口号（`9222`）、WebSocket URL（`ws://127.0.0.1:9222/...`）或 HTTP URL（`http://localhost:9222`）。适用于需要手动登录或绕过检测的场景。
...
┌──────────────────────────────────────────────────────────────────┐
│                         CLI Commands                              │
│           generate.ts  /  crawl.ts  /  export.ts                  │
└──────┬──────────────────────┬────────────────────┬───────────────┘
       │                      │                    │
 ┌─────▼──────┐        ┌─────▼──────┐      ┌─────▼──────┐
 │ ParserGen  │        │ CrawlEngine│      │  Database  │
 │  Service   │        │ (串行/并行) │      │  Service   │
 └─────┬──────┘        └─────┬──────┘      │  (SQLite)  │
       │                      │             └────────────┘
 ┌─────▼──────┐        ┌─────▼──────┐
 │ CodePost   │        │  Registry  │
 ...
 └─────┬──────┘              │
       │              ┌──────▼───────┐
 ┌─────▼──────┐       │  BaseParser  │ ◄── Parser / GenericParser
 │ LLMService │       │  (核心方法)   │
 │ (业务逻辑) │       └──────┬───────┘
 └─────┬──────┘              │
       │                     │
...
1. **生成阶段** (`npm run generate`)
   - 导航到目标 URL → 滚动页面触发懒加载 → 获取列表页可访问性树快照 + refs
   - 自动点击第一个职位链接 → 捕获详情页快照 + `getMainContentText()` 原始文本 → 导航回列表页
   - 如果 LLM 无法从快照识别职位链接，回退到 Cheerio HTML 解析检测
   - 将列表页快照 + 详情页快照/原始文本 + Prompt 发送给 LLM → 生成站点专用 JS 解析器
   - CodePostProcessor 对生成的代码做 AST 验证（Acorn）+ 6 种常见错误自动修复
   - 验证不通过时，将错误信息反馈给 LLM 重试（最多 2 次）
...
2. **爬取阶段** (`npm run crawl`)
   - 自动加载匹配的解析器（Registry 评分匹配：域名 +100、页面类型 +50）→ 无匹配时回退到 GenericParser（LLM 在线提取）
   - 导航到目标页面，支持串行模式（`concurrency=1`，复用单浏览器）或并行模式（`concurrency>1`，p-limit 浏览器池）
...
| `scrollPage()` | 滚动页面触发懒加载 |
| `dismissCookieBanner()` | 多策略关闭弹窗（CSS 选择器 + 文本匹配） |
| `connectCDP(url)` | 通过 CDP 连接已打开的 Chrome 实例 |

> **反检测**: BrowserService 启动时自动注入浏览器指纹伪装 — 禁用 `AutomationControlled`、自定义 User-Agent、伪造 `navigator.webdriver`/`plugins`/`languages`。
...
│   │   ├── CrawlConfig.ts     # 爬取/生成配置
│   │   └── LinksConfig.ts     # 链接配置
│   ├── parsers/               # 解析器
│   │   ├── base/
│   │   │   ├── BaseParser.ts  # 基类（collectJobLinks, extractDetailFields 等）
│   │   │   ├── GenericParser.ts # LLM 在线提取回退解析器
│   │   │   └── Parser.ts      # Parser 接口
│   │   ├── generated/         # LLM 自动生成的解析器（.js）
│   │   ├── helpers/           # 解析器辅助工具
│   │   └── registry.ts        # 解析器动态注册/加载（评分匹配）
│   ├── prompts/               # LLM Prompt 模板
│   │   ├── parser-generator.ts # System + User prompt（含列表页/详情页指导）
│   │   └── job-link-identifier.ts # 职位链接识别 prompt
│   ├── services/              # 服务层
│   │   ├── BrowserService.ts  # Playwright 浏览器封装（含 CDP / 反爬 / 弹窗处理）
│   │   ├── CrawlEngine.ts    # 爬取引擎（串行/并行模式）
│   │   ├── CodePostProcessor.ts # AST 验证 + 自动修复（Acorn）
│   │   ├── DatabaseService.ts # SQLite 数据库服务（WAL 模式 + 去重）
│   │   ├── JDExtractor.ts    # JD 结构化提取
│   │   ├── LLMClient.ts      # LLM API 传输层（重试 + 指数退避）
│   │   ├── LLMService.ts     # LLM 业务逻辑（页面分析/解析器生成/数据提取）
│   │   ├── ParserGenerator.ts # 解析器生成编排（快照/重试/持久化）
│   │   └── ParserPersistence.ts # 解析器文件持久化
│   └── utils/                 # 工具函数
│       ├── config.ts          # 全局配置（超时/重试/限制 + 域名→公司映射）
│       ├── jobId.ts           # job_id 生成工具（URL 提取 + MD5 回退）
│       ├── loadLinksCsv.ts    # CSV 文件加载
│       └── parserFilename.ts  # 解析器文件名生成（防冲突）
├── output/                    # 输出目录
│   ├── jobs.db                # SQLite 数据库（爬取结果）
│   ├── parsers/               # 生成的解析器（含调试信息）
│   │   └── <domain>/
│   │       ├── parser.js          # 生成的解析器代码
│   │       ├── snapshot.json      # 列表页快照
...
| [架构优化](docs/ARCHITECTURE_OPTIMIZATION.md) | 架构优化和重构说明 |
| [解析器跳过优化](docs/PARSER_SKIP_OPTIMIZATION.md) | 解析器缓存跳过策略 |
