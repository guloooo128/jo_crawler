# jpmc-fa-oraclecloud-com-list-sites-cx-1001

## 基本信息

- **域名**: jpmc.fa.oraclecloud.com
- **URL**: https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001/jobs?keyword=intern
- **页面类型**: list
- **生成时间**: 2026-02-05T10:00:06.525Z

## 文件说明

- **parser.js**: 自动生成的解析器代码
- **src/parsers/generated/jpmc-fa-oraclecloud-com-list-sites-cx-1001.js**: 用于动态加载的副本
- **snapshot.json**: 页面快照（可访问性树和 refs）
- **screenshot.png**: 页面截图
- **generation.log**: 生成过程的详细日志

## 使用方法

```javascript
// 方式1: 从 output/parsers 导入
import Parser from './output/parsers/jpmc-fa-oraclecloud-com-list-sites-cx-1001/parser.js';

// 方式2: 自动加载（系统会从 src/parsers/generated 加载）
const parser = new Parser();
const jobs = await parser.parse(browser, { maxItems: 10 });
```

## 元数据

```json
{
  "domain": "jpmc.fa.oraclecloud.com",
  "url": "https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001/jobs?keyword=intern",
  "pageType": "list",
  "generatedAt": "2026-02-05T10:00:06.525Z"
}
```

---

**注意**: 此文件和解析器代码由 AI 自动生成，手动修改可能会被覆盖。
